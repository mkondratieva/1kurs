...
{//через массив индексов	
	int a[11],n=11,n_ind=n,ind[11],	left,right,mid=0;
	srand(time(NULL));   
	for(int i=0;i<n;i++){a[i]=rand()%21; printf("%d ",a[i]);}
	n_ind=0;
	for(int i=0;i<n;i++)ind[n_ind++]=i;//занесли массив индексов
	for(int i=0;i<n_ind;i++){ 
		int flag=0; 
		for(int j=0;j<n;j++)
			if(ind[i]!=j&&cmp(a[ind[i]],a[j])){ 
				flag=1;
				 break;
			}
		if(flag==0)ind[i]=-1;//пометили как -1 индексы элементов, которые должны остаться в массиве а
	}
	for(left=0,right=0; right<n_ind;right++) //2 бегунка, удаляем помеченные элементы, остались те, которые надо удалить из а
		if(ind[right]>=0) ind[left++]=ind[right];
	n_ind=left;
	for(left=0,right=0;right<n;right++){ //3 бегунка: удаляем из а элементы, индексы которых встречаются в ind
		if(mid<n_ind&&ind[mid]==right){mid++;continue;}
		a[left++]=a[right];
	}
	n=left;
	puts("\nres");
...
//элементы массива, индексы r которых <i уже обработаны (если a[k]=-1, то его нужно будет удалить из массива, иначе a[k] не является близким никакому элементу исходного массива
	...
	puts("\n"); //те, которые стоят на неч. местах, остаются, на чет ост. те, кот. ост. в (2) 
}
	return 0;
} 