...
void foo(int (*)[N]); //функция с аргументом p: указатель на массив  элементов int размера N
void foo(int (*p)[N]){  // p указывает на массив  элементов типа int размера N
	(*p)[0]=-1; //*p -- массив элементов типа int размера N, (*p)[0] -- его первый элемент
}
//более короткий вариант
void foo1(int [N]); //функция с аргументом p:  массив  элементов int размера N
void foo1(int p[]){  // p -- ммассив  эдементов типа int размера N
	p[0]=-2; //p -- массив эдементов типа int размера N, p[0] -- его первый элемент
}
...
int main(void){
	...
	//эти значения совпадают, что позволяет передавать массив в функцию по ссылке
	printf("dif sum\n%p\n%p\n",(void *)(p+1),(void *)(mass+1));
//но арифметика указателя на массив и указателя на первый элемент разная!
	foo(p);foo(&mass);//передаем в функцию адрес массива, оба варианта верные, нельзя foo(mass)!
	pr(mass,n); 
	foo1(mass);//передаем адрес первого элемента, нельзя foo1(p)!
	pr(mass,n); 
	switch(err){
